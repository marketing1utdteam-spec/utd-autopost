...
def tt_publish(video_path, caption, dry=False):
    ...
    """
    # 🔴 ПОРОЖНІЙ ПІДПИС — ЦЕ ВІДМОВА, А НЕ ДРІБНИЦЯ. Причину «чернетка без `post_info`»
    # полагоджено 10.09.2026, але дірка лишалась поруч: якщо `caption` приїде порожнім
    # або з самих пробілів, ми пошлемо `title: ""`, і TikTok знову підставить назву
    # застосунку. Заміряно очима на профілі 17.09.2026 — два таких пости з 40 висять
    # публічно досі: підпис рівно «#UTDPublisher», без тексту, посилання й хештегів.
    #
    # Відмовитись дешево (відео лишиться в черзі й піде наступним прогоном), а
    # опублікований нікчемний пост не відкотити: прямої публікації в нас немає, і
    # видаляти чуже опубліковане я не буду. Тому межа стоїть ДО мережі.
    if not (caption or "").strip():
        raise ValueError(
            "TikTok: порожній підпис — публікація скасована. TikTok підставив би "
            "назву застосунку («#UTDPublisher»), і вийшов би пост без тексту, "
            "посилання й хештегів. Перевір генератор підписів (ttcaptions).")
