...
def main():
    ...
    # Тепер локальний прогін бере САМЕ ці типи (`--only th_post,li_post`), а
    # хмара — решту. Множини не перетинаються, тому подвоїти публікацію неможливо.
    #
    # 🔴 `tt_post` ЗВІДСИ ПРИБРАНО 11.09.2026, і цей рядок теж — він називав його ще
    # 22.09, тобто одинадцять діб підказував тип, який сюди більше не належить.
    # TikTok публікує `tiktok_publish.py` через Buffer; довідка — нижче, у гілці kind.
    ap.add_argument("--only", default="",
                    help="лише ці типи, через кому: th_post,li_post")
    ...
    # 🔴 СТЕЛІ ЗНИЖЕНІ 08.09.2026 за рішенням власника: «треба зменшити постінг, ми вже
    # нормально розгребли чергу». Було 9 постів на добу разом, стало 5.
    #
    # Чому саме такі числа, а не «менше навмання»:
    #   instagram 2 → 1  щодня, але один. Два на добу з однієї теки виглядають як
    #                    конвеєр; один тримає присутність і не читається як бот.
    #   youtube   4 → 2  Shorts люблять частоту, тому не 1. Але 4 на добу зʼїдали запас
    #                    за 12 діб — це найшвидший витік контенту з усіх мереж.
    #   facebook  3 → 1  органічний показ у FB низький незалежно від частоти; три пости
    #                    на добу там не дають утричі більше, а контент витрачають.
    #   threads       1  темп задає розклад (3 на тиждень), стеля лишається запобіжником
    #   linkedin      1  сторінка компанії втрачає показ при щоденному постингу
    #   tiktok        1  заявка ще на розгляді
    #
    # Стеля — це ЗАПОБІЖНИК, а не план. План задають дати в `schedule.json`; стеля не
    # дає прогону висипати чергу за раз, якщо дати збились. Тому знижувати стелю без
    # переставляння дат безглуздо: черга просто поїде з відставанням.
    # Стеля НА ОДИН пост вище темпу: пропущена доба (Mac спав, токен моргнув) наздожене
    # себе наступного дня й не поїде відставанням на місяці. Вище не ставимо — інакше
    # прогін висипле чергу пачкою, а пачка з однієї теки читається як бот.
    # 🔴 INSTAGRAM 2 → 1, І ТЕМП 7/тиждень → 3/тиждень. Рішення власника 23.09.2026
    # «зроби це», за заміром 22.09: охоплення 42 людини з 1103 підписників (3,9%),
    # вісім постів поспіль без реакцій, підписників меншає 1110 → 1103 за 10 діб.
    # Серед тих, хто ПОБАЧИВ, реагують 13–17% — утричі краще за норму. Отже ні зміст,
    # ні формат не винні (carousel 45, reels 41 — однаково), зламаний розподіл.
    # 🔴 Механізм, через який темп шкодить: кожен пост без реакції каже платформі, що
    # контент нецікавий цій аудиторії, і наступному показу дають меншу групу. Тобто
    # нарощувати темп на такому акаунті — гасити охоплення, а не розганяти.
    # Дати в schedule.json переставлені на Пн/Ср/Пт; стеля 1 лишається запобіжником,
    # щоб пропущена доба не поїхала пачкою. Запас контенту виріс 10.12 → 24.03.
    DAILY = {"instagram": env_int("DAILY_IG", 1),
             "youtube":   env_int("DAILY_YT", 2),
             "facebook":  env_int("DAILY_FB", 2),
             "threads":   env_int("DAILY_TH", 1),
             # 🔴 Стеля 1 → 2 за рішенням власника 09.09.2026: «робимо кожен день по
             # посту а то і по два якщо що бо в нас багато матеріалів».
             #
             # Що це скасовує, щоб було видно: тут стояла 1 із приміткою «сторінка
             # компанії втрачає показ при щоденному постингу». Власник вирішив інакше,
             # і це його рішення — я лишаю примітку, а не свою оцінку.
             #
             # 🔴 І головне: стеля тут НЕ вузьке місце. Заміряно того ж дня — у git 318
             # тек із матеріалом, але `caption_li.txt` є лише в 9. Тобто підняти стелю
             # без підписів під LinkedIn не дасть нічого: черга просто впирається в
             # відсутність тексту, а не в ліміт.
             "linkedin":  env_int("DAILY_LI", 2),
             # 🔴 TIKTOK НА ПАУЗІ: стеля 0, а не 1. Рішення власника, дослівно: «нам для
               # ТікТок не треба поки бо він заблокований», і «Чекаємо» про вердикт
               # застосунку (подано 04.09.2026).
               #
               # Нуль — це той самий спосіб паузи, що вже вживається для цілі `alertmail`:
               # не видалення, а нуль, який легко повернути. І це ОДИН ВЛАСНИК ФАКТА
               # «мережа на паузі»: тривога про запас контенту читає ту саму стелю, а не
               # тримає власний список винятків.
               #
               # Без цього запас TikTok дорівнював нулю назавжди, і тривога «Контент
               # закінчується» горіла ВІЧНО на мережі, у яку ми навмисно не постимо —
               # рівно та фальшива тривога, через яку червоне перестають читати.
               # 🔴 З 0 НА 1 — 10.09.2026, ПІСЛЯ ДОВЕДЕНОГО ЖИВОГО ПРОГОНУ.
               # Було 0 навмисно: застосунок стояв на розгляді, і публікувати було нічим.
               # Тепер: схвалено 10.09 09:01, продакшн-токен отримано, тестова чернетка
               # доїхала до інбоксу `utd_ecommerce` за 17 секунд (publish_id
               # v_inbox_file~v2.768383855…). Один на добу — рішення власника, і воно
               # обґрунтоване: TikTok жорсткіший за решту мереж до однотипних
               # завантажень, а їхнє власне обмеження «5 користувачів за 24 години»
    ...
    for e in queue:
        ...
        try:
            ...
            # виглядав би як «нічого не було в черзі».
                ...
                # 🔴 ЦЕЙ ПУБЛІКАТОР TikTok БІЛЬШЕ НЕ ВЕДЕ. Закрито 22.09.2026.
                #
                # З 10.09.2026 TikTok публікує `tiktok_publish.py` через Buffer —
                # маршрут без тапів, доведений живим публічним постом. Прямий клієнт
                # лишається неаудованим і віддав би SELF_ONLY: зелений статус,
                # справжній publish_id і нуль глядачів.
                #
                # Гілка тут була жива й досяжна: `--only` за замовчуванням порожній, а
                # умова відбору `not only or kind in only` при порожньому `--only`
                # пропускає ВСІ типи. Тобто голий `python3 run_due.py` пішов би старим
                # маршрутом. Це не гіпотеза: у ніч на 11.09.2026 локальний прогін уже
                # переписав `posted.json` зі своєї памʼяті, затер позначку про
                # опублікований reel01 — і старий маршрут спробував би видати те саме
                # відео вдруге, на публічний профіль бренду.
                #
                # `autopost_local.sh` передає `--only th_post,li_post`, тож штатний шлях
                # був безпечний. Але безпека, яка тримається на аргументі виклику,
                # ламається першим викликом без аргументу. Тому відмова тут, у коді.
                print("  ⏸ tt_post цим публікатором не публікується з 10.09.2026: "
                      "TikTok іде через Buffer (`tiktok_publish.py --fill`). "
                      "Запис уже стоїть у черзі Buffer або стане туди за 3 доби до дати.")
                skipped_by_cap += 1
                tried -= 1        # це не спроба, тому не палимо лічильник карантину
                continue
