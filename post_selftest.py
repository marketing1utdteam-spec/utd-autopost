...
def main():
    ...
    if "--no-net" not in sys.argv:
        ...
        # 🔴 у падіння беремо лише 🔴 (мережа ВІДМОВИЛА кодом), не 🟡 (не додзвонився).
        dead = [k for k, v in h.items() if v.startswith("🔴") and k != "tiktok"]
        shaky = [k for k, v in h.items() if v.startswith("🟡")]
        if shaky:
            say(f"   🟡 не доперевірив через мережу: {', '.join(shaky)} — це НЕ «мертвий»")
